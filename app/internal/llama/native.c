...
static bool peanut_abort_callback(void *user_data) {
	return atomic_load_explicit((atomic_bool *) user_data, memory_order_relaxed);
}
...
peanut_llama_status peanut_llama_open(const char *model_path, int32_t threads, peanut_llama **out) {
	...
	*out = NULL;
	...
	struct peanut_llama *engine = calloc(1, sizeof(*engine));
	if (engine == NULL) {
...
static peanut_llama_status peanut_append_piece(struct peanut_llama *engine, const struct llama_vocab *vocab, llama_token token) {
	...
	if (piece_len < 0) {
		...
		char *grown = realloc(engine->output, engine->output_len + (size_t) piece_len + 1);
		if (grown == NULL) {
			free(buffer);
			return PEANUT_LLAMA_OUTPUT_FAILED;
		}
...
peanut_llama_status peanut_llama_generate(
	...
	*output_len = 0;
	...
	llama_token *prompt_tokens = malloc((size_t) prompt_tokens_len * sizeof(*prompt_tokens));
	...
	struct llama_sampler *grammar = llama_sampler_init_grammar(vocab, schema, "root");
	if (grammar == NULL) {
		free(prompt_tokens);
		peanut_free_sampler(engine);
		return PEANUT_LLAMA_GRAMMAR_FAILED;
	}
	llama_sampler_chain_add(engine->sampler, grammar);
	...
	if (llama_model_has_encoder(engine->model)) {
		if (llama_encode(engine->context, batch) != 0) {
			free(prompt_tokens);
			peanut_free_sampler(engine);
			return atomic_load(&engine->abort_requested) ? PEANUT_LLAMA_CANCELLED : PEANUT_LLAMA_DECODE_FAILED;
		}
	...
	for (int32_t generated = 0; generated < max_tokens; generated++) {
		...
		if (decode_status != 0) {
			free(prompt_tokens);
			peanut_free_sampler(engine);
			return atomic_load(&engine->abort_requested) || decode_status == 2 ? PEANUT_LLAMA_CANCELLED : PEANUT_LLAMA_DECODE_FAILED;
		}
	...
	free(prompt_tokens);
	peanut_free_sampler(engine);
	if (atomic_load(&engine->abort_requested)) {
